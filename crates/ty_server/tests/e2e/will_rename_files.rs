use crate::TestServerBuilder;
use insta::assert_json_snapshot;

#[test]
fn simple_rename() -> anyhow::Result<()> {
    let mut server = TestServerBuilder::new()?
        .with_file("old_module.py", "")?
        .with_file("consumer.py", "")?
        .build()
        .wait_until_workspaces_are_initialized();

    server.open_text_document("old_module.py", "x = 1\n", 1);
    server.open_text_document(
        "consumer.py",
        "import old_module\n\nprint(old_module.x)\n",
        1,
    );

    let old_uri = server.file_uri("old_module.py").to_string();
    let new_uri = server.file_uri("new_module.py").to_string();

    let edits = server.will_rename_files(vec![lsp_types::FileRename { old_uri, new_uri }]);

    assert_json_snapshot!(edits);

    Ok(())
}

#[test]
fn directory_rename() -> anyhow::Result<()> {
    let mut server = TestServerBuilder::new()?
        .with_file("old_pkg/__init__.py", "")?
        .with_file("consumer.py", "")?
        .build()
        .wait_until_workspaces_are_initialized();

    server.open_text_document("old_pkg/__init__.py", "x = 1\n", 1);
    server.open_text_document("consumer.py", "from old_pkg import x\n\nprint(x)\n", 1);

    let old_uri = server.file_uri("old_pkg").to_string();
    let new_uri = server.file_uri("new_pkg").to_string();

    let edits = server.will_rename_files(vec![lsp_types::FileRename { old_uri, new_uri }]);

    assert_json_snapshot!(edits);

    Ok(())
}

#[test]
fn document_changes_when_supported() -> anyhow::Result<()> {
    let mut server = TestServerBuilder::new()?
        .enable_workspace_edit_document_changes(true)
        .with_file("old_module.py", "x = 1\n")?
        .with_file("consumer.py", "import old_module\n")?
        .build()
        .wait_until_workspaces_are_initialized();

    let old_uri = server.file_uri("old_module.py").to_string();
    let new_uri = server.file_uri("new_module.py").to_string();
    let edits = server.will_rename_files(vec![lsp_types::FileRename { old_uri, new_uri }]);

    assert_json_snapshot!(edits);

    Ok(())
}

#[test]
fn py_and_pyi_batch_has_unique_edits() -> anyhow::Result<()> {
    let mut server = TestServerBuilder::new()?
        .with_file("old_module.py", "x = 1\n")?
        .with_file("old_module.pyi", "x: int\n")?
        .with_file("consumer.py", "import old_module\nprint(old_module.x)\n")?
        .build()
        .wait_until_workspaces_are_initialized();

    let edits = server.will_rename_files(vec![
        lsp_types::FileRename {
            old_uri: server.file_uri("old_module.py").to_string(),
            new_uri: server.file_uri("new_module.py").to_string(),
        },
        lsp_types::FileRename {
            old_uri: server.file_uri("old_module.pyi").to_string(),
            new_uri: server.file_uri("new_module.pyi").to_string(),
        },
    ]);

    assert_json_snapshot!(edits);

    Ok(())
}

#[test]
fn overlapping_directory_and_file_batch_has_unique_edits() -> anyhow::Result<()> {
    let mut server = TestServerBuilder::new()?
        .with_file("old_pkg/__init__.py", "")?
        .with_file("old_pkg/sub.py", "x = 1\n")?
        .with_file("consumer.py", "import old_pkg.sub\n")?
        .build()
        .wait_until_workspaces_are_initialized();

    let edits = server.will_rename_files(vec![
        lsp_types::FileRename {
            old_uri: server.file_uri("old_pkg").to_string(),
            new_uri: server.file_uri("new_pkg").to_string(),
        },
        lsp_types::FileRename {
            old_uri: server.file_uri("old_pkg/sub.py").to_string(),
            new_uri: server.file_uri("new_pkg/sub.py").to_string(),
        },
    ]);

    assert_json_snapshot!(edits);

    Ok(())
}
