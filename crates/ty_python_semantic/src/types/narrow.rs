...
impl<'db> PatternSuccessAnalyzer<'db> {
    ...
    fn analyze_successful_pattern(
        ...
        match pattern {
            ...
            PatternPredicateKind::Class(kind) => {
                let class_ty = necessary_match_pattern_type(self.db, self.program, pattern);
                let class =
    ...
    fn analyze_successful_or_pattern_arm(
        ...
        for pattern in patterns {
            remaining_subject_ty = pattern_fallthrough_type(
                self.db,
                self.program,
                previous_pattern,
        ...
        match subject_ty {
            ...
            Type::TypedDict(_) if filter_nominal_arms => {
                if class.is_some_and(|class| {
                    KnownClass::Dict.is_subclass_of(
                        self.db,
                        self.program,
    ...
    fn matched_subject_type_for_original(
        ...
        if matched_types.is_equivalent_to(self.db, self.program, filtering_types)
