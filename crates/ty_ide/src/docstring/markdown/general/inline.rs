...
/// Renders a supported subset of reST hyperlinks in prose.
///
/// The set of links we've chosen to support was informed by a point-in-time
/// review of inline links that appeared in a sample of popular Python repos
/// (including pytorch, scikit-learn, pandas, and numpy, amongst others).
/// The supported shapes covered >90% of the links we found. The goal is
/// therefore >90% coverage of links found in the wild without growing this into
/// a general reST parser. That percentage does not include separate link
/// families that we do not support, such as named references or relative
/// targets (which require Sphinx context that a standalone docstring parser
/// does not have).
///
/// # Examples
///
...
impl Renderer {
    ...
    fn render_line_with_pending_link(
        ...
        let continuation = if let Some(uri_prefix) = pending_link.uri_prefix() {
            let Some(continuation) = LinkContinuation::parse_fragment(uri_prefix, line.text) else {
                output.push_str(&pending_link.fallback);
                self.render_line_without_pending_link(output, line);
                return;
            };
            ...
            LinkContinuation::parse(line.text)
        };
...
/// A link (either complete or still pending a URI) found in inline text.
enum InlineLink<'a> {
    /// A complete hyperlink beginning at byte offset `start`.
    Complete { start: usize, link: Hyperlink<'a> },
    /// A wrapped label beginning at byte offset `start` and awaiting its target.
...
/// Outcome of parsing a possible reST link
enum Candidate<'a> {
...
///
/// Plausible wrapped labels without a closing backtick are `Pending`,
/// malformed or unsupported forms are `Rejected`, and all other supported
/// link are `Complete`.
...
#fragment>`_",
    ...
    fn renders_many_links() {
        let source = "`link <https://example.com>`_. ".repeat(3);
        let rendered = render_docstring(&source);
        assert_eq!(rendered.matches("[link](https://example.com)").count(), 3);
    }
